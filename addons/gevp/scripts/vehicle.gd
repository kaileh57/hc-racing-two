...
func process_throttle(delta: float):
	...
	if is_reverse:
		throttle_input = -abs(throttle_input)  # Ensure throttle is negative in reverse
	else:
		throttle_input = abs(throttle_input)  # Ensure throttle is positive in forward
...
func process_transmission(delta: float):
	...
	if automatic_transmission:
		...
		if not reversing:
			...
			if current_gear < gear_ratios.size():
				if current_gear > 0:
					if current_real_gear_rpm > 0.9 * max_rpm or current_ideal_gear_rpm > 0.9 * max_rpm:
						shift(1)
...
func set_reverse(reverse: bool) -> void:
	is_reverse = reverse
	if is_reverse:
		requested_gear = -1
	else:
